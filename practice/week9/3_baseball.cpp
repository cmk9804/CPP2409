...
bool checkNum(int checkingNum, int &first, int &second, int &third);
...
int main() {
	...
	while (1) {		
		...
		same = checkNum(randomNum, firstNum, secondNum, thirdNum);
		/*
	...
	while (1) {
		...
		while (1) {
			...
			same = checkNum(userNumber, guessFirst, guessSecond, guessThird);
			/*
			bool same = false; // 모든 수가 다를 경우 true를 갖는 변수
			// TODO 1: 3자리 숫자의 자릿수를 추출하고, 모든 수가 다른지 체크하는 코드블록 작성
...
bool checkNum(int checkingNum, int &first, int &second, int &third){
	...
	// TODO 1: 3자리 숫자의 자릿수를 추출하고, 모든 수가 다른지 체크하는 코드블록 작성
	first = checkingNum / 100;
	second = (checkingNum / 10) % 10;
	third = checkingNum % 10;
